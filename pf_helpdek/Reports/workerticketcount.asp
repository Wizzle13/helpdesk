...
<%
    ...
    sql = sql & " GROUP BY CALLS.CALL_SERVICED_BY, CALLS.CLOSED  HAVING (((CALLS.CLOSED)='NO'))  ORDER BY Count(CALLS.TICKET_NUMBER) DESC    "
    Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open sql, conn, 3, 3

    sqlcount = "SELECT Count(CALLS.TICKET_NUMBER) AS TotalCountOfTICKET_NUMBER  FROM CALLS WHERE CALLS.CLOSED='NO'"
    Set rec = Server.CreateObject("ADODB.Recordset")
    rec.Open sqlcount, conn, 3, 3


    strTotal=0
%>
<TABLE BORDER=1 BGCOLOR=#ffffff CELLSPACING=0 align=left Valign=top width=100%><B></B>

<THEAD>
<TR>
<TH BGCOLOR=#c0c0c0 BORDERCOLOR=#000000 >Person Working Call</TH>
<TH BGCOLOR=#c0c0c0 BORDERCOLOR=#000000 >Number of<BR>Open Tickets</TH>
<TH BGCOLOR=#c0c0c0 BORDERCOLOR=#000000 >Percentage of<BR>Open Tickets</TH>
...
<TD BORDERCOLOR=#c0c0c0  ALIGN=RIGHT><%=Round(Server.HTMLEncode(rs.Fields("CountOfTICKET_NUMBER").Value)/Server.HTMLEncode(rec.Fields("TotalCountOfTICKET_NUMBER").Value)*100, 2)%>%<BR></FONT></TD>
...
<TD BORDERCOLOR=#c0c0c0  ALIGN=RIGHT>100.0%<BR></FONT></TD>
